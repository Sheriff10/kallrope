   ...
   const handleSubmit = async (e) => {
      ...
      try {
         const response = await axios.post(
            `${window.api}/auth/driver/login`,
            data
         );
         console.log(response);
         if (response) {
            setLoading(false);
            const token = response.headers["driver-auth-token"];
            storeToken("driverAuthToken", token);
            navi("/driver/dashboard");
         }
      } catch (error) {
         if (error) {
            setLoading(false);
